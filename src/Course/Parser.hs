...
data ParseResult a =
  ...
  -- Result contains the remaining input, and the final result, a
  | Result Input a
  deriving Eq
...
mapParser ::
  ...
  -> Parser a
  -> Parser b
mapParser f parser = P (\i -> case parse parser i of
...
-- Result >< ""
--
-- TODO: understand how this works for examples above.
list ::
  Parser a
  -> Parser (List a)
list p = list1 p ||| (valueParser Nil)
-- Alternative using bindParser:
-- list p = (bindParser (\a -> bindParser (\as -> valueParser (a :. as)) (list p)) p) ||| (valueParser Nil)
--
-- Laborious, manual way (what happens underneath the hood):
-- list p = P (\i -> case parse p i of
--                   ErrorResult e -> Result i Nil
--                   Result i' a   -> case parse (list p) i' of
--                                    ErrorResult _ -> Result i' (a :. Nil)
--                                    Result i'' a' -> Result i'' (a :. a'))

-- | Return a parser that produces at least one value from the given parser then
-- continues producing a list of values from the given parser (to ultimately produce a non-empty list).
--
-- That is, this parser tries to force at least one value, at the expense with
-- being ok if it errors. It disallows producing empty lists.
--
...
list1 ::
  Parser a
  -> Parser (List a)
list1 p = bindParser (\a -> bindParser (\as -> valueParser (a :. as)) (list p)) p
-- Easier to understand:
--
-- list1 p = bindParser (\a -> P (\i -> case parse (list p) i of
--                                      Result i' a' -> Result i' (a :. a')
--                                      ErrorResult e -> ErrorResult e)) p
...
natural ::
  Parser Int
natural = bindParser (\ds -> case read ds of
...
sequenceParser =
  foldRight
    (\parser sumParser ->
      ...
      flbindParser sumParser (\as -> valueParser (a :. as))))
    (valueParser Nil)
...
firstNameParser ::
  Parser Chars
firstNameParser = upper `flbindParser` (\u -> (list lower) `flbindParser` (\rest -> valueParser (u :. rest)))
...
surnameParser =
  ...
  thisMany 5 lower     `flbindParser` (\us ->
  list lower           `flbindParser` (\uss ->
  valueParser (u :. us ++ uss))))
...
personParser = do
  ageV       <- ageParser
  _          <- spaces1
  firstNameV <- firstNameParser
  _          <- spaces1
  surnameV   <- surnameParser
  _          <- spaces1
  smokerV    <- smokerParser
  _          <- spaces1
  phoneV     <- phoneParser
  pure $ Person ageV firstNameV surnameV smokerV phoneV
-- Manually:
-- personParser =
  -- ageParser       `flbindParser` (\ageV ->       spaces1 >>>
  -- firstNameParser `flbindParser` (\firstNameV -> spaces1 >>>
  -- surnameParser   `flbindParser` (\surnameV ->   spaces1 >>>
  -- smokerParser    `flbindParser` (\smokerV ->    spaces1 >>>
  -- phoneParser     `flbindParser` (\phoneV ->
