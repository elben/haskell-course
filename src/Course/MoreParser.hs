...
-- True
--
-- TODO: Why is this reversed? How does this work?!!
string ::
  Chars
  -> Parser Chars
string = traverse is
-- string s = error "above it doesn't work, is wrong way"
...
noneof ::
  Chars
  -> Parser Char
noneof chars = satisfy (\c -> notElem c chars)
...
-- Result >< 'a'
--
-- >>> (parse (between (string "[") (string "]") (string "abc")) "[abc]")
-- Result >< "abc"
--
...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok ca cb = between (charTok ca) (charTok cb)
...
hex = do
  ...
  return $ chr (case readHex digits of
    Full n -> n
    Empty -> 0)
